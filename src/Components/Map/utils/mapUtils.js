...
export const fetchCoordinates = async (place, accessToken) => {
  const geocodingUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
    place
  )}.json?access_token=${accessToken}`;
...
export const stateLevel = (
  ...
  const clusterProperties = clusterPropertiesObject(UAEStates);
  const clusterCountArray = clusterCountFilterArray(UAEStates);
...
export const countryLevel = (
  ...
  // Example usage:
  const nextLevel = incrementAndFormat(endRange);
  const country = ["==", ["get", "country"], "United Arab Emirates"];
